...
def mineral(input_file, output_file):
    ...
    with (
        open(input_file, mode="r", encoding="utf-8") as f_input,
        ...
        for row in reader:
            ...
            try:
                ...
                print(
                    f"Warning: valor de análise química inválido para {mol}: {content_value}. Pulando."
                )
                continue
            ...
            if mass == 0:
                print(
                    f"Warning: não foi possível calcular a massa molar de {mol}. Pulando."
                )
                continue

            if abs(balance) > 1e-6:
                print(
                    f"Warning: molécula {mol} apresenta balanço de carga não nulo ({balance}). Gravando saída mesmo assim."
                )
