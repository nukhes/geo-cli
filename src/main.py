...
def workflow(processor):
    try:
        processor
        typer.secho("sucesso! arquivo final criado", fg=typer.colors.GREEN, bold=True)
...
def geo2utm(
    ...
    source_epsg: str = typer.Option(
        "EPSG:4326", help="EPSG code for source coordinate system"
    ),
    target_epsg: str = typer.Option(
        "EPSG:31983", help="EPSG code for target coordinate system"
    ),
):
    """
    ...
    workflow(
        conv.GeoUtm(
            f_input=str(input_file),
            f_output=str(output_file),
            epsg_source=source_epsg,
            epsg_target=target_epsg,
        ).run()
    )
...
def utm2geo(
    ...
    source_epsg: str = typer.Option(
        "EPSG:31983", help="EPSG code for source coordinate system"
    ),
    target_epsg: str = typer.Option(
        "EPSG:4326", help="EPSG code for target coordinate system"
    ),
):
    """
...
def map(
    ...
    id: str = typer.Argument(
        None,
        help="nome da coluna a ser usada como identificador dos pontos no mapa (opcional)",
    ),
...
def mineralformula(
    ...
    workflow(formula.mineral(input_file, output_file))


@app.command()
def escala(
    escala: str = typer.Argument(..., help="escala cartográfica"),
    distancia_carta: float = typer.Argument(
        ..., help="distância medida na carta (em cm)"
    ),
):
    """
    ...
    typer.secho(
        f"distância real: {res} cm ou {res / 1000} km", fg=typer.colors.GREEN, bold=True
    )


@app.command()
def indicecor(
    olivina: float = typer.Option(
        0.0, "--olivina", "-o", help="Porcentagem visual de Olivina"
    ),
    piroxenio: float = typer.Option(
        0.0, "--piroxenio", "-p", help="Porcentagem visual de Piroxênio"
    ),
    biotita: float = typer.Option(
        0.0, "--biotita", "-b", help="Porcentagem visual de Biotita"
    ),
    anfibolio: float = typer.Option(
        0.0, "--anfibolio", "-a", help="Porcentagem visual de Anfibólio"
    ),
    opacos: float = typer.Option(
        0.0,
        "--opacos",
        help="Percentagem de minerais metálicos/opacos (Magnetite, Ilmenite)",
    ),
    outros: float = typer.Option(
        0.0,
        "--outros",
        help="Outros minerais ferromagnesianos (Granada, Turmalina, etc.)",
    ),
):
    """
    ...
    if m > 1.0:
        typer.secho(
            f"a soma das porcentagens de minerais escuros excede 100% (m = {m}). O índice de cor pode ser impreciso.",
...
@app.command()
def mergulho(
    length: float = typer.Argument(
        ..., help="comprimento do afloramento na superfície"
    ),
    ...
    typer.secho(
        f"espessura real: {maps.real_thickness(length, angle)}",
...
def idaderocha(
    ...
    daughter: float = typer.Option(
        ..., "--filho", "-f", help="quantidade do isótopo filho"
    ),
    hl: float = typer.Option(
        ..., "--meia-vida", "-mv", help="meia-vida do isótopo pai"
    ),
):
    """
    ...
    if parent <= 0:
        typer.secho(
            "a quantidade do isótopo pai deve ser maior que zero.", fg=typer.colors.RED
        )
        raise typer.Exit(code=1)
